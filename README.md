...
- **重大な問題として、Git管理下においてskillsからdisabled-skillsディレクトリに移動（オフ）してからコミットせずにPullするなどでスキルがオンとオフどちらにも重複してしまう問題があります。オン・オフを切り替えたときにコミットが必要であることを警告として出すようにしました。Git管理状態を認識します。**
